var Dashboard = {
  template : `
<div>
  <v-layout justify-center column v-if="groups.loaded">
    <v-expansion-panel popout>
      <v-expansion-panel-content v-for="(group, i) in allGroups" :key="i" hide-actions>
  ...
  <div v-else>Hold on, loading groups...</div>
</div>
`,
  data: function() {
    return {
  ...
  computed: {
    ...
    allGroups: function() {
      var all = [];
      for(var g in this.groups.data) {
        all.push(this.groups.data[g]);
      }
      for(var g in this.more_groups) {
        all.push(this.more_groups[g]);
      }
      return all;
    }
  },
